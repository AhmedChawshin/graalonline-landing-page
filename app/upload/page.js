...
export default function Upload() {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('type2', formData.uploadType);
    data.append('guild', '');
    ...
    try {
      ...
      if (message.includes("Unknown file type") || message.includes("Please enter the correct Upload Code") || message.includes("enough") || message.includes("Sorry there was a problem with submitting the image!")  || message.includes("Message not found") || message.includes("You need to specify a correct e-mail address or Graal ID!") || message.includes("email") || message.includes("There is no player with the account")) {
        toast({
